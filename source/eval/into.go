...
func IntoBool(vm *vm.VM, val *object.Value) (bool, object.ExpressionResult) {
	...
	res := Call(vm, method.SignalVal.Normalize(), nil)
	if res.Signal.Has() {
		return false, res
	}
...
func IntoIter(val *object.Value, vm *vm.VM) object.ExpressionResult {
	if AttrExists(val.Normalize(), keywords.Next) {
		return object.ExpressionResult{SignalVal: val}
	}
	...
	res := Call(vm, method.SignalVal.Normalize(), nil)
	if res.Signal.Has() || AttrExists(res.SignalVal.Normalize(), keywords.Next) {
		return res
	}
	return object.ExpressionResult{
		...
		SignalVal: &object.Value{InnerValue: object.Error{
			Code:    errors.TypeError,
			Message: fmt.Sprintf("expected method '%s'", keywords.Next),
		}},
	}
}
