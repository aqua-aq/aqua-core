...
func main() {
	...
	newArray := eval.RunBin(vm, scope, false, array, num(6), operators.Plus)
	if newArray.Signal.Has() {
		panic(newArray)
	}
	*array = *newArray.SignalVal
	elem := eval.RunBin(vm, scope, false, array, num(1), operators.Index)
	if elem.Signal.Has() {
		panic(elem)
	}
	newElem := eval.RunBin(vm, scope, false, elem.SignalVal, num(1), operators.Plus)
